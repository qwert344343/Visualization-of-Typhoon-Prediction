...
服务启动后，同一 WiFi 下的手机/平板用浏览器直接访问电脑的局域网 IP，页面会自动切换为移动端抽屉布局：

```bash
ipconfig           # 查看电脑"IPv4 地址"，如 192.168.1.100
# 手机浏览器打开 http://192.168.1.100:8080
```

无法访问时依次排查：① 防火墙是否放行 `node.exe`；② 校园网/企业网可能开启 AP 隔离（禁止设备互访），可让电脑连接手机热点后改用热点网络的 IP 访问。
